    ...
    with db_table.batch_writer() as batch_writer:
        ...
        for record in iter_deaggregate_records(raw_kinesis_records):
            ...
            try:
                ...
                input_num_actions_per_watermark = str(
                    json_document['num_actions_per_watermark'])

                # DYNAMODB LAYER:
                # - Add time as Monitor Control and Write micro-batch to DynamoDB:
                json_document['ddb_partition_key'] = 'userid#{}#appserver#{}'.format(
                    input_user_id, 'app-server-tomcat-123')
                json_document['ddb_sort_key'] = int(
                ...
                if int(input_num_actions_per_watermark) > 10:
                    ...
                    print('Email notification sent, due high severity incident. API Response: {}'.format(
                        sns_response))
